    ...
    for _ in [None] * retry:
        try:
            if num_image == 1:
                ...
                logging.info(f"save to {local_path}")
                video = cv2.VideoWriter(local_path, fourcc, save_fps, (width, height))
                ...
                for fid, frame in enumerate(images):
                    ...
                    tpth = os.path.join(frame_dir, '%04d.png' % (fid+1))
                    cv2.imwrite(tpth, frame[:,:,::-1], [int(cv2.IMWRITE_JPEG_QUALITY), 100])
                # logging.info(frame_dir)
                image_files = Path(frame_dir)
                image_files = sorted(image_files.iterdir())
